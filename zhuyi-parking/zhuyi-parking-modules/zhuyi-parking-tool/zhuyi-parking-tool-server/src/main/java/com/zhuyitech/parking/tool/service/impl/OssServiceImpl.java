...
import com.zhuyitech.parking.tool.config.QiNiuConfig;
...
public class OssServiceImpl implements OssService {
    ...
    public ObjectResultDto<OssFileUploadResultDto> qiNiuOssFileUpload(InputStream inputStream) {
        ...
        String accesskay = qiNiuConfig.getAccesskey();
        String secretkay = qiNiuConfig.getSecretekey();
        //要上传的空间
        String bucketname = qiNiuConfig.getBucket();
        //七牛的默认域名
        String url = qiNiuConfig.getUrl();
        try {
    ...
    public ObjectResultDto<OssFileUploadResultDto> aliyunOssFileUpload(String fileName, InputStream inputStream) {
        ...
        if (null == inputStream || StringUtils.isEmpty(aliYunConfig.getBucket())) {
            return resultDto.failed();
        }
        OssFileUploadResultDto ossFileUploadResultDto = new OssFileUploadResultDto();
...
//            objectMetadata.setContentDisposition("inline;filename=" + fileName);
    ...
    private OSSClient createOssClient() {
        ...
        if (StringUtils.isEmpty(aliYunConfig.getEndpoint())
                || StringUtils.isEmpty(aliYunConfig.getAccessKeyId())
