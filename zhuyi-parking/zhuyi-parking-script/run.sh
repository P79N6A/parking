...
MODULE_SERVER_NAME=${SERVICE_PATH##*/}
MODULE_NAME=${MODULE_SERVER_NAME%-impl*}
EXECUTE_JAR_NAME=${MODULE_SERVER_NAME}.jar
...
getpid() {
    ...
    pid=`ps -ef | grep "$MODULE_SERVER_NAME" | grep -v "$0" | grep -v "grep" |  awk '{print $2}'`
}
...
start() {
    ...
    if [ -n "$pid" ]; then 
        ...
        echo "Error during $MODULE_SERVER_NAME starting, see log for details."
fi
}
...
debug() {
    ...
    if [ -n "$pid" ]; then 
        ...
        echo "[DEBUG]Error during $MODULE_SERVER_NAME starting, see log for details."
    fi
}
...
stop() {
    ...
    if [ -n "$pid" ]; then
        ...
        echo
        if ! [ -n "$pid" ]; then 
	    echo "$MODULE_SERVER_NAME has been successfully stopped."
        else 
	    echo "Error during $MODULE_SERVER_NAME stopping... $res"
        fi
    fi
}
...
status() {
    getpid
    if [ -n "$pid" ]; then 
        echo "$MODULE_SERVER_NAME (pid $pid) is running..."
    else 
        echo "$MODULE_SERVER_NAME is NOT running"
    fi
}
