  ...
  <div class="bill-query-list-box" ref="orderDOM">
    ...
    <ul class="order-list" v-if="orderList.length">
      <li v-for="(item, idx) in orderList" :key="idx">
        <singleBill :bill="item" :isQuick=true></singleBill>
      </li>
...
<script>
import {scanQueryList} from '@/utils/api'
...
export default {
  ...
  created () {
    ...
    localStorage.getItem('v2_userInfo') && this.$store.dispatch('saveUserInfo', JSON.parse(localStorage.getItem('v2_userInfo')))
    this.queryTpl.plateNumber = localStorage.getItem('plateNumber')
    this.queryTpl.noncestr = localStorage.getItem('userState')
    this.queryTpl.plateColor = localStorage.getItem('queryPlateColor') * 1
    this.getData()
  },
  methods: {
    getData () {
      scanQueryList(this.queryTpl).then(res => {
        if (res.data.code === 1) {
          this.loading = false
