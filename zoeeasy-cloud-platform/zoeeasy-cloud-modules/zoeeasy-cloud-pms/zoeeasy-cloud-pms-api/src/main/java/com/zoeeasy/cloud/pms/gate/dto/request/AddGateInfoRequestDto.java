...
@ApiModel(value = "AddGateInfoRequestDto", description = "添加停车场出入口请求参数")
public class AddGateInfoRequestDto extends SignedRequestDto {
     ...
     * 管理系统出入口编号
     */
    @ApiModelProperty("gateCode")
    @NotBlank(message = ParkingGateConstant.PARKING_GATE_CODE_NOT_NULL)
    @Length(min = ParkingGateConstant.GATE_CODE_MIN_LENGTH,
            max = ParkingGateConstant.GATE_CODE_MAX_LENGTH,
            message = ParkingGateConstant.GATE_CODE_LENGTH_RANGE)
    @Pattern(regexp = ParkingGateConstant.GATE_CODE_PARTTERN,
