 ...
 */
@Service
@RocketMQMessageListener(topic = MessageQueueDefinitions.Topic.PASSING_VEHICLE,
        consumerGroup = MessageQueueDefinitions.ConsumeGroup.HIKVISION_PASSING)
@Slf4j
public class PassingVehicleMessageConsumer implements RocketMQListener<PassingVehiclePayload> {
    ...
    public void onMessage(PassingVehiclePayload passingVehiclePayload) {
        if (passingVehiclePayload != null) {
            ...
            try {
                ...
                messageFail.setDestination(MessageQueueDefinitions.Topic.PASSING_VEHICLE_FAIL);
    ...
    private String getPassVehicleLockKey(PassingVehiclePayload passVehicle) {
        ...
        return
                new StringBuilder().append("lock:zoeeasy.passing.hikvision.pass_").
